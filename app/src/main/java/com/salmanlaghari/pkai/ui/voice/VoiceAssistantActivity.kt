...
class VoiceAssistantActivity : AppCompatActivity() {
    ...
    private fun initializeTts() {
        binding.tvAnimated_status.text = "Initializing engines..."
        ...
        recognitionHelper = VoiceRecognitionHelper(this, object : VoiceRecognitionHelper.Callback {
            override fun onReadyForSpeech() {
                runOnUiThread {
                    ...
                    binding.tvAnimated_status.text = "Listening... Speak now"
                }
            }

            override fun onBeginningOfSpeech() {
                runOnUiThread {
                    binding.tvAnimated_status.text = "Recording your thoughts..."
                }
            }
            ...
            override fun onPartialResults(text: String) {
                runOnUiThread {
                    if (text.isNotBlank()) {
                        binding.tvAnimated_status.text = text
                    }
                }
            }
            ...
            override fun onResults(text: String) {
                runOnUiThread {
                    if (text.isNotBlank()) {
                        ...
                        binding.tvAnimated_status.text = "Try speaking again..."
                        setAssistantState(AssistantState.LISTENING)
                    }
                }
            }
            ...
            override fun onError(error: String) {
                runOnUiThread {
                    binding.tvAnimated_status.text = error
    ...
    private fun setupButtons() {
        ...
        applyTouch(binding.btn_clear_transcript)
        applyTouch(binding.btn_stop_toggle)
        applyTouch(binding.btn_confirm_send)

        binding.btn_clear_transcript.setOnClickListener {
            binding.containerTranscriptBubbles.removeAllViews()
            binding.tvAnimated_status.text = "Transcript cleared."
        }

        binding.btn_stop_toggle.setOnClickListener {
            playToneFeedback()
            when (currentState) {
                AssistantState.LISTENING -> {
                    recognitionHelper?.stopListening()
                    binding.tvAnimated_status.text = "Processing voice..."
        ...
        binding.btn_stop_toggle.setOnLongClickListener {
            finish()
            true
        }

        binding.btn_confirm_send.setOnClickListener {
    ...
    private fun submitToAiModel(prompt: String) {
        setAssistantState(AssistantState.PROCESSING)
        binding.tvAnimated_status.text = "Thinking..."
        ...
        lifecycleScope.launch {
            try {
                ...
                binding.tvAnimated_status.text = "Error: ${e.localizedMessage}"
                setAssistantState(AssistantState.LISTENING)
            }
        }
    }
    ...
    private fun speakResponse(text: String) {
        ...
        binding.tvAnimated_status.text = ""
        ttsActiveJob?.cancel()
        ttsActiveJob = lifecycleScope.launch {
            ...
            for (word in words) {
                ...
                binding.tvAnimated_status.text = sb.toString()
